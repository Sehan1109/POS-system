    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4">
            ...
            <form method="GET" class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 flex flex-wrap gap-3 items-end">
                ...
                <a href="{{ route('admin.activity-logs.index') }}" class="px-4 py-2 text-sm text-gray-500 hover:text-gray-700">Reset</a>
            </form>
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                            <td class="px-6 py-3 text-xs text-gray-500 whitespace-nowrap">{{ $log->created_at->format('d M Y H:i:s') }}</td>
                            <td class="px-6 py-3 text-sm text-gray-900 dark:text-white">{{ $log->user?->name ?? 'System' }}</td>
                            <td class="px-6 py-3">
                                <span class="px-2 py-0.5 text-xs rounded-full font-semibold {{ $actionColors[$log->action] ?? 'bg-gray-100 text-gray-600' }}">
                                    {{ $log->action }}
                                </span>
                            </td>
                            <td class="px-6 py-3 text-xs text-gray-500">{{ $log->model_type ?? '—' }}{{ $log->model_id ? " #{$log->model_id}" : '' }}</td>
                            <td class="px-6 py-3 text-sm text-gray-700 dark:text-gray-300 max-w-xs truncate">{{ $log->description }}</td>
                            <td class="px-6 py-3 text-xs text-gray-400 font-mono">{{ $log->ip_address ?? '—' }}</td>
                        </tr>
                        @empty
                        <tr><td colspan="6" class="px-6 py-10 text-center text-gray-400">No activity logs found.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
