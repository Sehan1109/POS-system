        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-5 mb-7">
                <div class="rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700">
                    ...
                    <p class="text-3xl font-bold text-slate-900 dark:text-white">${{ number_format($data['total_sales'], 2) }}</p>
                ...
                <div class="rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700">
                    ...
                    <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $data['total_products'] }} /
                        {{ $data['low_stock'] }}</p>
                ...
                <div class="rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700">
                    ...
                    <p class="text-3xl font-bold text-slate-900 dark:text-white">{{ $data['total_users'] }} /
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-7">
                <a href="{{ route('admin.users.index') }}"
                    class="group rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 transition hover:-translate-y-0.5 hover:shadow-md">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Admin Action</p>
                    <h3 class="mt-2 text-lg font-semibold text-slate-900 dark:text-white">User Management</h3>
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Create and manage staff accounts.</p>
                ...
                <a href="{{ route('admin.products.index') }}"
                    class="group rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 transition hover:-translate-y-0.5 hover:shadow-md">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Admin Action</p>
                    <h3 class="mt-2 text-lg font-semibold text-slate-900 dark:text-white">Inventory</h3>
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Track items, prices, and stock levels.</p>
                    <span class="mt-3 inline-block text-sm font-medium text-cyan-700 group-hover:text-cyan-800">Open
                        module -></span>
                </a>
                ...
                <a href="{{ route('admin.reports.sales') }}"
                    class="group rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 transition hover:-translate-y-0.5 hover:shadow-md">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Admin Action</p>
                    <h3 class="mt-2 text-lg font-semibold text-slate-900 dark:text-white">Sales Reports</h3>
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Analyze daily and monthly performance.</p>
                    <span class="mt-3 inline-block text-sm font-medium text-cyan-700 group-hover:text-cyan-800">Open
                        module -></span>
                </a>
                ...
                <a href="{{ route('admin.activity-logs.index') }}"
                    class="group rounded-2xl bg-white dark:bg-slate-800 p-5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-700 transition hover:-translate-y-0.5 hover:shadow-md">
                    <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Admin Action</p>
                    <h3 class="mt-2 text-lg font-semibold text-slate-900 dark:text-white">Activity Logs</h3>
                    <p class="mt-1 text-sm text-slate-600 dark:text-slate-400">Audit user and system actions quickly.</p>
                    <span class="mt-3 inline-block text-sm font-medium text-cyan-700 group-hover:text-cyan-800">Open
                        module -></span>
                </a>
            </div>
                            ...
                            <tbody>
                                @forelse ($recentSales as $sale)
                                    <tr class="border-b border-slate-100 dark:border-slate-700/50">
                                        ...
                                        <td class="py-2 pr-3 text-slate-700 dark:text-slate-300">{{ $sale->user?->name ?? 'N/A' }}</td>
                                        <td class="py-2 pr-3">
                                            <span
                                                class="px-2 py-1 rounded-full text-xs font-medium {{ $sale->status === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' }}">
                                                {{ ucfirst(str_replace('_', ' ', $sale->status)) }}
                                            </span>
                                        </td>
                                        <td class="py-2 text-right font-semibold text-slate-900 dark:text-white">
                    ...
                    <div class="space-y-3">
                        @forelse ($lowStockProducts as $product)
                            <div class="flex items-center justify-between rounded-xl border border-slate-200 dark:border-slate-700 p-3">
                                ...
                                <span
                                    class="px-2 py-1 rounded-full text-xs font-semibold {{ $product->stock_quantity <= 0 ? 'bg-rose-100 text-rose-700 dark:bg-rose-900/30 dark:text-rose-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' }}">
                                    {{ $product->stock_quantity }} in stock
                                </span>
                            </div>
                        @empty
                            <p class="text-sm text-slate-500 dark:text-slate-400">All products have healthy stock levels.</p>
                        @endforelse
                    </div>

                    <div class="mt-5 grid grid-cols-2 gap-3 text-center">
                        <div class="rounded-xl bg-slate-50 dark:bg-slate-900/50 p-3">
                            <p class="text-xs text-slate-500 dark:text-slate-400">Out of Stock</p>
                            <p class="text-xl font-bold text-rose-600 dark:text-rose-400">{{ $data['out_of_stock'] }}</p>
                        </div>
                        <div class="rounded-xl bg-slate-50 dark:bg-slate-900/50 p-3">
                            <p class="text-xs text-slate-500 dark:text-slate-400">Pending Refunds</p>
